...
초기 schema는 `migrations/0001_metadata.sql`이며, 원격 D1에 적용했습니다.
...
`DB` 바인딩이 없으면 기존처럼 Vectorize metadata만 사용합니다. `DB` 바인딩이 있으면 `keyframes` 테이블의 `label`, `floor`, `zone`, `image_r2_key`, `x`, `y`가 Vectorize 결과를 보강합니다.
